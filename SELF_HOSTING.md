...
| Issue | Fix |
|-------|-----|
| 🌐 Nothing on port 3000 / `worldmonitor` Exited (2) | Rebuild after pull: `podman compose build --no-cache worldmonitor && podman compose up -d worldmonitor`. Check logs: `podman logs worldmonitor`. Common cause: Alpine nginx missing `gzip_static` module (fixed in current Dockerfile). |
